...
默认每天晚上 **9:00（北京时间）** 自动运行。

说明：

- 当前使用 GitHub Actions 自带 `schedule`。
- GitHub 官方调度在高峰期可能存在延迟，所以改到 21:00 是为了尽量保证你能在 22:00 前收到日报。
- 如果你后面需要“严格 22:00 准点”，那就还是要改成外部精确触发方案。
...
### Q2: 为什么晚上 10 点前后没有准时收到日报？

这是因为项目当前使用的是 GitHub Actions 自带的 `schedule`，而它不是严格实时调度，存在延迟触发的情况。

当前仓库已经把默认执行时间提前到了 **21:00（北京时间）**，目的是给 GitHub 的调度延迟预留缓冲时间，尽量保证你在 **22:00 前** 收到日报。

如果你需要“严格到点”的效果，那么单靠 GitHub 自带定时并不能完全保证。
